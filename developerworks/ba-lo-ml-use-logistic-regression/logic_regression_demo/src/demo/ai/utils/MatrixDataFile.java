...
public class MatrixDataFile {
	...
	private boolean featureScaling = true ; 			// 特征是否缩放 Feature Scaling
	...
	public void featureScaling() throws Exception{
		...
		for(int col=0;col<cols;col++) {
			...
			/*
			System.out.println(" >> 第 " + col + " 列，缩放参数：");
	...
	public static void main(String args[]) throws Exception {
		...
		MatrixDataFile datafile = new MatrixDataFile("/dk/java/AI/data/uci/LineRegTest.csv");
		datafile.featureScaling();
		ArrayList<float[][]> data = datafile.get();
		float[][] x = data.get(0);
		float[][] y = data.get(1);
		
		
		System.out.println("x:");
		System.out.println(Matrix.matrixToString(x).toString());
		
		System.out.println("y:");
		System.out.println(Matrix.matrixToString(y).toString());
